...
void ethernetcode_init(void){//16080 lines counting the 5 comments at the top
	...
	while (fgets( s, 300, in_file ) != NULL)
	{
		...
		for(j = 7; j < 300; j++)
		{
			...
			routerCompany[j-7] = c;//we start adding the company name here
			//if(companyStart == 1)
			//{
		...
		//char * value = routerID;
		//if (1 == sscanf(value, "0x%"SCNx32, &prefix)) {
		...
		//printf("%x \n", prefix);
		//printf("%s \n", routerCompany);
		//char * routerPointer = routerID;
		//sscanf(scanf("%x", &a)
		//%x used for reading in hexadecimal in scanf
		//prefix = stringToHex(&routerPointer);
		//prefix = atoi(routerID);
		currentCode->prefix = prefix;
		currentCode->vendor = routerCompany;//convert this to
