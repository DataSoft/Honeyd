...
%token	CREATE ADD BCAST PORT BIND CLONE DOT FILTERED OPEN CLOSED DEFAULT SET ACTION
...
;
newbcast    : BCAST template PORT NUMBER NUMBER action
	{
		struct action *action;
		if ($2 == NULL) {
			yyerror("No template");
